...
| Input monitoring | Windows WinMM joystick axes, optional `pynput` scroll deltas, and HIDAPI MI_03 special reports. Pointer coordinates are intentionally discarded. Controlled trials are explicitly labelled paddle-only or wheel-only. | HID special-report monitoring is present; it does not provide Windows Raw Input or Linux `evdev` telemetry. | Hidraw special-event monitoring is present; it does not provide a cross-platform relative-input telemetry layer. |
